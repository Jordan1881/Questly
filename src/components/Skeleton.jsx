...
export function SkeletonCard({ className = '' }) {
  return (
    <div
      className={`bg-white rounded-[12px] border border-[#e5e7eb] p-5 ${className}`}
      aria-busy="true"
      aria-label="Loading"
    >
  ...
  return (
    <div className={`flex flex-col gap-3 ${className}`} aria-busy="true" aria-label="Loading">
      {Array.from({ length: count }, (_, i) => (
        <div key={i} className="border border-[#e5e7eb] rounded-[8px] px-5 py-4 flex items-center gap-3">
...
export function SkeletonRewardGrid({ count = 4, className = '' }) {
  return (
    ...
    >
      {Array.from({ length: count }, (_, i) => (
        <div key={i} className="bg-white border border-[#e5e7eb] rounded-[12px] p-5 flex flex-col gap-3">
